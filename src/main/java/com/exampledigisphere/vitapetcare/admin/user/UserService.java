...
@Service
@Transactional
@Info(dev = Info.Dev.heltonOliveira, label = Info.Label.feature, date = "06/02/2026",
...
public class UserService {
  ...
  public UserService(final UserRepository userRepository, final WorkDayService workDayService,
  ...
  public Optional<UserDTO> register(UserDTO input, Set<UserAssociations> associations) {
    ...
    return UserFactory.createFrom(input).map(userRepository::save)
        .map(usr -> usr.applyAssociations(transformToAssociationSet(associations)))
        .map(UserFactory::toResponse).map(savedUser -> persistDependencies(input, savedUser));
  }
  ...
  public Optional<UserDTO> retrieve(Long id, Set<UserAssociations> associations) {
    ...
    return userRepository.findById(id)
        .map(usr -> usr.applyAssociations(transformToAssociationSet(associations)))
        .map(usr -> initialize(usr, associations)).map(UserFactory::toResponse);
  }
  ...
  public Page<UserDTO> list(@NonNull Pageable pageable) {
    ...
    return userRepository.findAll(pageable).map(UserFactory::toResponse);
  }
